...
class TestReviewSession:
    ...
    def test_init_with_default_values(self, sample_deck):
        ...
        session = ReviewSession(deck_id=sample_deck.id, user_id="user-1")
    ...
    def test_load_due_cards(self, sample_deck, reference_time):
        ...
        session = ReviewSession(
            deck_id=sample_deck.id, user_id="user-1", current_time=reference_time
        )
    ...
    async def test_next_card_empty_queue(self, sample_deck):
        ...
        session = ReviewSession(deck_id=sample_deck.id, user_id="user-1", cards=[])  # Empty queue
