...
def test_flashcard_crud_operations(test_db):
    ...
    card = Flashcard(front="Test Front", back="Test Back", deck_id=deck.id)
...
def test_get_due_flashcards(test_db):
    ...
    card1 = Flashcard(front="Due Card", back="Back", deck_id=deck.id, due_date=past_date)

    # Card that is not yet due
    card2 = Flashcard(front="Not Due Card", back="Back", deck_id=deck.id, due_date=future_date)

    # Card with no due date (should be treated as due)
    card3 = Flashcard(front="No Due Date Card", back="Back", deck_id=deck.id)
