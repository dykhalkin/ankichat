...
class CSVService:
    ...
    def export_all_decks(self, user_id: str, output_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not decks:
                ...
                return {
                    "success": False,
                    "message": "You have no decks to export",
                    "files": []
                }
            ...
            return {
                "success": False,
                "message": f"Error exporting decks: {str(e)}",
                "files": []
            }
    ...
    async def import_with_auto_deck(
        ...
        try:
            ...
            temp_deck = self.deck_repo.create(Deck(
                name="Temp Import Deck",
                description="Temporary deck for CSV import",
                user_id=user_id
            ))
            
            import_result = self.import_from_csv(
                file_path, user_id, temp_deck.id, False
            )
            ...
            for i in range(0, len(imported_cards), batch_size):
                ...
                for card in batch:
                    ...
                    suggested_name = await self.llm_client.detect_category(content) if self.llm_client else "Imported Cards"
            ...
            for card in imported_cards:
                ...
                if deck_name in deck_name_to_id:
                    ...
                    # Create new deck
                    new_deck = self.deck_repo.create(Deck(
                        name=deck_name,
