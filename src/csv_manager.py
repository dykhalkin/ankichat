...
DECK_CSV_HEADERS = ["id", "name", "description", "created_at", "user_id"]
...
class CSVManager:
    ...
    def __init__(self, deck_repo: DeckRepository, flashcard_repo: FlashcardRepository):
        """
    ...
    def export_deck_to_csv(self, deck_id: str, output_path: str) -> str:
        ...
        safe_name = (
            "".join(c for c in deck.name if c.isalnum() or c in "_ ").strip().replace(" ", "_")
        )
    ...
    def import_flashcards_from_csv(
                    ...
                    try:
                        ...
                        if not deck_id:
                            ...
                            if deck_name in deck_name_to_id:
                                ...
                                warnings.append(
                                    f"Skipped card: No valid deck for '{row.get('front', 'Unknown')}'"
                                )
                                continue
                        ...
                        try:
                            ...
                            warnings.append(
                                f"Invalid created_at format for card '{row.get('front', 'Unknown')}', using current time"
                            )
                        ...
                        try:
                            ...
                            warnings.append(
                                f"Invalid due_date format for card '{row.get('front', 'Unknown')}', will be calculated on review"
                            )
                        ...
                        warnings.append(
                            f"Error importing card '{row.get('front', 'Unknown')}': {e}"
                        )
        ...
        logger.info(
            f"Imported {cards_imported} cards and created {decks_created} decks from {file_path}"
        )
